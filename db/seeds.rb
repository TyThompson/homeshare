...
marias_users_homes = UserHome.create(:user_id => maria.id, :home_id => marias_home.id,
 :exp => Chore.where(chore_completer_id: maria.id, home_id: marias_home_id).pluck(:chore_xp).sum, :admin => "true")

tys_users_homes = UserHome.create(:user_id => ty.id, :home_id => tys_home.id,
 :exp => Chore.where(chore_completer_id: ty.id, home_id: tys_home_id).pluck(:chore_xp).sum, :admin => "true")
